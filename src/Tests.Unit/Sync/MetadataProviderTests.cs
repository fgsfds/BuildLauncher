...
public sealed class MetadataProviderTests : IDisposable
{
    ...
    public async Task UpdateAvailable_ZippedAddonWithDifferentManifest_ReturnsTrue()
    {
        var (provider, _) = await CreateProviderAsync(
            new List<AddonManifestJsonModel>
            {
    ...
    public async Task MetadataUpdatedEvent_FiresOnFolderUpdate()
    {
        ...
        var result = await provider.UpdateMetadataAsync(new(Path.Combine(ClientProperties.AddonsFolderPath, "UnpackedAddon"), "addon2.json"));
    ...
    public async Task UpdateMetadataAsync_WhenNotCached_ReturnsError()
    {
        ...
        var result = await provider.UpdateMetadataAsync(new(Path.Combine(ClientProperties.AddonsFolderPath, "ZippedAddon.zip"), "addon.json"));
    ...
    public async Task UpdateMetadataAsync_WhenFileDoesNotExist_ReturnsError()
    {
        ...
        var result = await provider.UpdateMetadataAsync(new AddonFilePathWrapper(Path.Combine(ClientProperties.AddonsFolderPath, "NonExistent.zip"), "addon.json"));
